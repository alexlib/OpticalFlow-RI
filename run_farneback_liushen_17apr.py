#!/usr/bin/env python
"""
Run Farneback optical flow algorithm with Liu-Shen enhancement on 17apr image pair
"""
...
def main():
    ...
    output_dir = 'farneback_liushen_17apr_results'
    os.makedirs(output_dir, exist_ok=True)
    
    # Define Farneback parameters
    window_size = 33
    poly_n = 7
    poly_sigma = 1.5
    iterations = 5
    ...
    fb_adapter = Farneback_PyCL(windowSize=window_size, Niters=iterations, polyN=poly_n, polySigma=poly_sigma)
    ...
    # Save flow data
    output_base = os.path.join(output_dir, "Farneback_PyrLvls2")
    save_flow(U, V, f"{output_base}.mat")
    
    # Plot results
    plot_results(U, V, "Farneback", f"{output_base}.png")
    ...
    # Save flow data
    output_base = os.path.join(output_dir, "Farneback_LiuShen_PyrLvls2")
    save_flow(U_ls, V_ls, f"{output_base}.mat")
    
    # Plot results
    plot_results(U_ls, V_ls, "Farneback with Liu-Shen Enhancement", f"{output_base}.png")
    
    # Plot difference between Liu-Shen and regular Farneback
    diff_U = U_ls - U
    diff_V = V_ls - V
    
    output_base = os.path.join(output_dir, "Farneback_LiuShen_Difference")
