"""
Test file for the denseLucasKanade_Numba implementation.
"""
...
def test_denseLucasKanade_Numba_compute_small():
    ...
    assert np.mean(U_out[13:20, 12:20]) > 1.5  # x-component should be around 2
    assert np.mean(V_out[13:20, 12:20]) > 2.5  # y-component should be around 3
...
def test_bilinear_interpolate():
    ...
    assert lk._bilinear_interpolate(img, 0, 0) == 1
    assert lk._bilinear_interpolate(img, 1, 0) == 2
    assert lk._bilinear_interpolate(img, 0, 1) == 4
    assert lk._bilinear_interpolate(img, 1, 1) == 5

    # Test interpolated values
    assert lk._bilinear_interpolate(img, 0.5, 0) == 1.5
    assert lk._bilinear_interpolate(img, 0, 0.5) == 2.5
    assert lk._bilinear_interpolate(img, 0.5, 0.5) == 3.0

    # Test out of bounds values
    assert lk._bilinear_interpolate(img, -1, 0) == 0
    assert lk._bilinear_interpolate(img, 0, -1) == 0
    assert lk._bilinear_interpolate(img, 3, 0) == 0
    assert lk._bilinear_interpolate(img, 0, 3) == 0
